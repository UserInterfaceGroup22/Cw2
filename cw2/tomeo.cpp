...
int main(int argc, char *argv[]) {
    ...
    //QVBoxLayout *layout2 = new QVBoxLayout();//changed - might be user
    buttonWidget->setLayout(layout);


    //line 108 connects the signal generated by the button to a slot of the player
        for ( int i = 0; i < 7; i++ ) { //changed to 7 instead of 4
            TheButton *button = new TheButton(buttonWidget);
            button->connect(button, SIGNAL(jumpTo(TheButtonInfo* )), player, SLOT (jumpTo(TheButtonInfo* ))); // when clicked, tell the player to play.
            buttons.push_back(button);
            button->setIconSize(QSize(130,130));
//          button->setContentsMargins(0,5,0,0);
...
//    layout2->addLayout(layout);
    // layout->setGeometry(); //for later
    window.setLayout(top);
    window.setWindowTitle("Outdoor Application");
    window.setMinimumSize(270, 545); //size for an iphone
    window.setMaximumSize(813, 1062);//size of an ipad
    ...
    std::cout << window.height() << "Test" << endl;
